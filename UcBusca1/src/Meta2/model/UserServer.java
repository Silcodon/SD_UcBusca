...
import Meta2.model.RMIInterface;
...
public class UserServer extends UnicastRemoteObject implements RMIInterface {
	String MULTICASTGROUP="224.0.224.8";
	String RMIADRESS="224.0.224.9";
    ...
    //Metodo para mandar o termo da pesquisa e receber os urls
    public ArrayList<String> pesquisa_url(String pesquisa,String username){
        ArrayList<String> received=new ArrayList<String>();;
        String urls="";
        try{
            //Create the socket but we don't bind it as we are only going to send data
            ...
            //Enviar informação
            String msg="link "+username+" "+pesquisa;
            System.out.println(msg);
            byte[] buf= msg.getBytes();
            //Create Datagram Packet
            DatagramPacket pack_sender=new DatagramPacket(buf,buf.length,InetAddress.getByName(MULTICASTGROUP),PORT);
            ...
            System.out.println("Exception in UserServer.pesquisa_url: "+e);
        }
        byte[] buff=new byte[1024];
        try{
            MulticastSocket r=new MulticastSocket(PORT);
